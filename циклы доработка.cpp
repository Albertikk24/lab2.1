...
int main() {
    ...
    while (i < count) {  // проверка условия ДО выполнения тела цикла
        int n = n_values[i];
    ...
    i = 0;
    do {  // тело цикла выполняется ПРЕЖДЕ проверки условия
        int n = n_values[i];
        double P = P0 * pow(ratio, n);
        cout << n << "\t\t" << P << endl;
        i++;
    } while (i < count);  // проверка условия ПОСЛЕ выполнения тела цикла
